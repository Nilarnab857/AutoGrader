...
const {
  fetchTextData,
  compareStrings,
} = require("../Utils/fetchTextFromUrl.utils.js");
...
class AssigmentController {
        ...
        for (let roll_no = start; roll_no <= end; roll_no++) {
          try {
            ...
            if (user) {
              ...
              console.log(
                `User with roll number ${roll_no} not found or not matching role.`
              );
            }
          } catch (error) {
            console.error(
              `Error assigning assignment to user with roll number ${roll_no}: ${error.message}`
            );
          }
        }
      }
  ...
  async checkAssignments(assignment_id) {
    try {
      ...
      for (const submission of submissions) {
        ...
        if (!convertedFile || convertedFile.trim() === "") {
          console.error("Code is empty or invalid:", file);
        ...
        var marks = (a * 100.0) / testCaseSize;
        ...
        if (marks < 100) {
          const options = {
            ...
            data: {
              prompt:
                "The question of my problem is" +
                currAssignment.question +
                " Please can you check what is wrong with my solution code-->" +
                "code is" +
                convertedFile,
            },
          };
          ...
          const options = {
            ...
            data: {
              prompt:
                "The question of my problem is" +
                currAssignment.question +
                "This code is correct but can you provide feedback on what i can improve upon in the code-->" +
                "code is" +
                convertedFile,
            },
          };
      ...
      const checkedAssignments = await this.checkPlagiarism(
        language,
        newSubmissions,
        stringArr
      );
  ...
  async addSubmission(submission, assignment_id) {
    try {
      ...
      if (
        currAssignment.submissions.findIndex(
          (sub) =>
            sub.rollNumber.toString() === submission.rollNumber.toString()
        ) !== -1
      ) {
  ...
  async submitFeedback(feedback, roll_no, id) {
    try {
      ...
      const index = submissions.findIndex(
        (submission) => submission.rollNumber === roll_no
      );
      if (index === -1) {
